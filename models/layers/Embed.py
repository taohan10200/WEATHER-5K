...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, root_path, node_num, embed_type='fixed', freq='h', dropout=0.1):
        ...
        with open(f'{root_path}/meta_info.json') as f:
            geo_info = json.load(f)
        national_pos = []
        for k, v in geo_info.items():
            national_pos.append([v['latitude'], v['latitude'], v['ELEVATION']])
        national_pos = np.array(national_pos)
        
        se_idx = (national_pos.shape[0]//node_num)*node_num

        national_pos = national_pos[:se_idx,:]
