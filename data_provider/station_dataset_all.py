...
class Dataset_Weather_Stations_ALL(Dataset):
    ...
    def batch_sample(self, indexs): 
        ...
        if not self.registed_completed:
            ...
            need_load = [id for id in self.station_names if id not in self.registed_StationIndex]
        ...
        seq_x = np.array(copy.deepcopy(self.shared_data_np[:, s_begin:s_end,:]))
        seq_x = seq_x[:(seq_x.shape[0]//350)*350,:,:]
